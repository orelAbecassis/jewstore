...
class WPKites_Customizer_Notify_Section extends WP_Customize_Section {
	
	public $type = 'wpkites-customizer-notify-section';
	...
	public function call_plugin_api( $slug ) {
		...
		$call_api = get_transient( 'wpkites_cust_notify_plugin_info_' . $slug );
		if ( false === $call_api ) {
			...
			set_transient( 'wpkites_cust_notify_plugin_info_' . $slug, $call_api, 30 * MINUTE_IN_SECONDS );
		}
	...
	public function json() {
		...
		foreach ( $WPKites_Customizer_Notify_recommended_actions as $key => $wpkites_lite_customizer_notify_recommended_action ) {
			if ( $WPKites_Customizer_Notify_show_recommended_actions[ $wpkites_lite_customizer_notify_recommended_action['id'] ] === false ) {
				continue;
			}
			if ( $wpkites_lite_customizer_notify_recommended_action['check'] ) {
				continue;
			}

			$wpkites_lite_customizer_notify_recommended_action['index'] = $key + 1;

			if ( isset( $wpkites_lite_customizer_notify_recommended_action['plugin_slug'] ) ) {
				$active = $this->check_active( $WPKites_Customizer_Notify_recommended_action['plugin_slug'] );
				$wpkites_lite_customizer_notify_recommended_action['url'] = $this->create_action_link( $active['needs'], $wpkites_lite_customizer_notify_recommended_action['plugin_slug'] );
				if ( $active['needs'] !== 'install' && $active['status'] ) {
					$wpkites_lite_customizer_notify_recommended_action['class'] = 'active';
				} else {
					$wpkites_lite_customizer_notify_recommended_action['class'] = '';
				}

				switch ( $active['needs'] ) {
					case 'install':
						$wpkites_lite_customizer_notify_recommended_action['button_class'] = 'install-now button';
						$wpkites_lite_customizer_notify_recommended_action['button_label'] = $install_button_label;
						break;
					case 'activate':
						$wpkites_lite_customizer_notify_recommended_action['button_class'] = 'activate-now button button-primary';
						$wpkites_lite_customizer_notify_recommended_action['button_label'] = $activate_button_label;
						break;
					case 'deactivate':
						$wpkites_lite_customizer_notify_recommended_action['button_class'] = 'deactivate-now button';
						$wpkites_lite_customizer_notify_recommended_action['button_label'] = $deactivate_button_label;
						break;
				}
			}
			$formatted_array[] = $wpkites_lite_customizer_notify_recommended_action;
		}// End foreach().
		...
		foreach ( $WPKites_Customizer_Notify_recommended_plugins as $slug => $plugin_opt ) {

			if ( ! $plugin_opt['recommended'] ) {
				continue;
			}

			if ( isset( $wpkites_lite_customizer_notify_show_recommended_plugins[ $slug ] ) && $wpkites_lite_customizer_notify_show_recommended_plugins[ $slug ] ) {
				continue;
			}
		...
		$json['total_actions']       = count( $WPKites_Customizer_Notify_recommended_actions );
		...
		<# if( data.recommended_actions.length > 0 || data.recommended_plugins.length > 0 ){ #>
			<li id="accordion-section-{{ data.id }}" class="accordion-section control-section control-section-{{ data.type }} cannot-expand">
				...
				<h3 class="accordion-section-title">
					...
					<# if( data.recommended_actions.length > 0 ){ #>
						<span class="wpkites-customizer-plugin-notify-actions-count">
							<span class="current-index">{{ data.recommended_actions[0].index }}</span>
							{{ data.total_actions }}
						</span>
					<# } #>
				</h3>
				<div class="wpkites-theme-recomended-actions_container" id="plugin-filter">
					<# if( data.recommended_actions.length > 0 ){ #>
						<# for (action in data.recommended_actions) { #>
							<div class="wpkites-recommeded-actions-container epsilon-required-actions" data-index="{{ data.recommended_actions[action].index }}">
								<# if( !data.recommended_actions[action].check ){ #>
									<div class="wpkites-epsilon-recommeded-actions">
										<p class="title">{{ data.recommended_actions[action].title }}</p>
										<span data-action="dismiss" class="dashicons dashicons-no wpkites-customizer-notify-dismiss-recommended-action" id="{{ data.recommended_actions[action].id }}"></span>
					...
					<# if( data.recommended_plugins.length > 0 ){ #>
						<# for (action in data.recommended_plugins) { #>
							<div class="wpkites-recommeded-actions-container epsilon-recommended-plugins" data-index="{{ data.recommended_plugins[action].index }}">
								<# if( !data.recommended_plugins[action].check ){ #>
									<div class="wpkites-epsilon-recommeded-actions">
                                                                                    <?php if (!is_rtl()) { ?>
                                                                                        <p class="title">{{ data.recommended_plugins[action].title }}</p>
                                                                                        <span data-action="dismiss" class="dashicons dashicons-no wpkites-customizer-notify-dismiss-button-recommended-plugin" id="{{ data.recommended_plugins[action].id }}"></span>
                                                                                    <?php } else {
                                                                                        ?>
                                                                                        <span data-action="dismiss" class="dashicons dashicons-no wpkites-customizer-notify-dismiss-button-recommended-plugin" id="{{ data.recommended_plugins[action].id }}"></span>
                                                                                        <p class="title">{{ data.recommended_plugins[action].title }}</p>
