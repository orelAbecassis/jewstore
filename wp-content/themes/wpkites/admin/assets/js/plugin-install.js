jQuery(document).ready(function($) {
    'use strict';
    var this_obj = wpkites_companion_install;
    ...
    $(document).on('click', '.action-watch', function(event) {
        ...
        if(action_id){
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'wpkites_update_rec_acts', action_id:action_id},
                success:function(data) {
    ...
    $(document).on('click', '.spicethemes-customizer-notification-dismiss', function(event) {
        ...
        var sdata = {};
        if(slug){
            sdata.action = 'wpkites_hide_customizer_notice';
            sdata.wpkites_plugin_slug = slug;
        }else{
            sdata.action = 'wpkites_hide_customizer_companion_notice';
        }
