...
class Themify_Enqueue_Assets {
	...
	echo '<!--css_position--><script> </script>';//bug  in chrome https://bugs.chromium.org/p/chromium/issues/detail?id=332189x
    }
	...
	if(!in_array('Themify_Enqueue_Assets::getBuffer',ob_list_handlers(),true)){
    ...
    public static function getBuffer($body){
	$key = '';
	...
	if(strpos($body,'<!--svg_holder-->')!==false){
	    $body=str_replace('<!--svg_holder-->',self::loadIcons(false),$body);
	}
	else{
	    $body.=self::loadIcons(false);
	}
	self::$googleFonts =  null;
	return strpos($body,'<!--css_position-->')!==false?str_replace('<!--css_position-->',$path,$body):$path.$body;
    }
